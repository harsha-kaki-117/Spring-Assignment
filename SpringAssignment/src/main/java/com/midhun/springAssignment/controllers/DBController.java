package com.midhun.springAssignment.controllers;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RestController;

import com.midhun.springAssignment.model.PersonsModel;

@RestController
public class DBController {
	
	@Autowired
	DBService db;
	
	@RequestMapping(method=RequestMethod.POST,value="/assign")
	public void PostPersons(@RequestBody PersonsModel body) {
		db.addPersons(body);
	}
	
	@RequestMapping(method=RequestMethod.GET,value="/get")
	public List<PersonsModel> getAllPersons() {
		return db.getPersons();
	}
	
	@RequestMapping(method=RequestMethod.GET,value="/get/{id}")
	public PersonsModel getOnePerson(@PathVariable int id) {
		return db.getOnePerson(id);
	}
	
	@RequestMapping(method=RequestMethod.PUT,value="assign/{id}")
	public void updatePersons(@RequestBody PersonsModel body, @PathVariable int id) {
		db.updatePersons(body,id);
	}
	
	@RequestMapping(method=RequestMethod.DELETE, value="/delete/{id}")
	public void deletePersons(@PathVariable int id) {
		db.deletePersons(id);
	}
	
	@RequestMapping("/dummy")
	public String getDummy() {
		return "This is dummy check";
	}
	
}
